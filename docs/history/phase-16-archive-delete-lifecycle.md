...
- No new credentials. No recipe content in logs. **Actor is not auditable after the fact** — `auth.uid()` reauthorizes each RPC call at the time it runs, but nothing persists it: no `archived_by`/`deleted_by`/`restored_by` column exists on `recipes`, the `deleted_recipes` tombstone (ADR-0013) carries only `id`/`household_id`/`deleted_at`, and no pgaudit/statement logging is configured in `supabase/config.toml` to capture it another way. This diverges from established precedent elsewhere in this codebase — `planning_entries.added_by` (Phase 12) and `cooking_events.cooked_by` (Phase 15) both capture the acting user via a `uuid references auth.users(id)` column populated from `auth.uid()` on write — and leaves Phase 16's own security-checklist bullet "Auditable actor and identifiers without recipe contents" unsatisfied (Codex review, PR #53). Disclosed under Known limitations below, not treated as evidence for this Pass.
...
- **No auditable actor for archive/delete/restore actions.** The five lifecycle RPCs (`archive_recipe`, `unarchive_recipe`, `delete_recipe`, `restore_recipe`, `permanently_delete_recipe`) reauthorize via `auth.uid()` at call time but store no actor anywhere; after any of these calls there is no way to determine which household member performed it. This is a real divergence from established precedent, not an app-wide unaddressed pattern — `planning_entries.added_by` (Phase 12) and `cooking_events.cooked_by` (Phase 15) both already capture the acting user on write, the same shape Phase 16 would need (`archived_by`/`deleted_by`/`restored_by` on `recipes`, plus an actor column on the `deleted_recipes` tombstone). Leaves Phase 16's own security bullet "Auditable actor and identifiers without recipe contents" unsatisfied. Flagged by Codex (PR #53); not fixed here — carried forward, developer's call whether it changes this Pass.
