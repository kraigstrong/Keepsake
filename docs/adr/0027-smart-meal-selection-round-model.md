...
- **A retry resumes rather than collides.** The unique index covers `status = 'active'` only, so an abandoned `pending_candidates` row never blocks anything; `create_selection_round` adopts and returns an existing `pending_candidates` round for the same household and creator instead of minting a second one, and the Edge Function simply re-runs scoring and finalize against it.
- **Concurrent creation still resolves to one winner.** Two racing attempts both reach finalize; the unique index admits exactly one transition into `active` and the loser fails cleanly, which is the same fencing outcome ADR-0020 gets from an atomic status-guarded `UPDATE`.
