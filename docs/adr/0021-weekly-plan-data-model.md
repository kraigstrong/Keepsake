...
Phase 12 builds This Week (prd.md §15): an ordered, shared household shortlist with a Planning state (add/reorder/remove) and a Confirmed state (locked order), per the visual spec in the developer's "Keepsake Visual Directions" design doc (claude.ai/design project `7c61aad2-...`, same Ink & Paper system as ADR-0009 — colors/type already match `src/theme/tokens.ts` exactly, no new tokens needed). The design doc's Confirmed mockup also shows a "Ready for groceries? → Export" banner, but grocery export doesn't exist until Phase 13 (execution-plan.md's Phase 12 build scope itself has no "offer export" bullet) — deliberately left out of this phase rather than shipping a banner whose only action goes nowhere; Phase 13 is the natural place to add it once Export is real.
...
**Remove + Undo is client-side only, no soft-delete column.** "Undo" is a local banner (screen-scoped, not the app-wide `Toast`, which has no action-button concept) holding the just-removed entry's data for a few seconds; tapping it calls `add_to_weekly_plan` again rather than restoring a tombstoned row — the re-added entry lands at the end of the list, not necessarily back at its original position, since `add_to_weekly_plan` only ever appends (a documented simplification, not a bug: re-ordering it back exactly would need a second `reorder_planning_entries` call for a few-seconds-long affordance nothing else needs that precision for). Nothing in the PRD implies Undo must survive an app restart, and this avoids a second deletion-state concept alongside Phase 9's outbox/Phase 5's tombstones this feature doesn't otherwise need.
