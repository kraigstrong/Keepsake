...
- **Enforce the category vocabulary at the schema/decoding level, not just the prompt.** Found during `reliability/category-mapping-prompt`'s own review (2026-08-19): `RecipeExtractionSchema.suggestedCategories` (`server/ai/extractRecipe.ts`) is still `z.array(z.string())` even though the real category list is now available at call time — the closed set is enforced only by prompt instruction plus `mapCategoryNamesToIds`' fuzzy post-hoc match, not by Anthropic's structured-output constrained decoding, which a per-request `z.array(z.enum(categoryValues))` would get for free (confirmed: Zod v4's `z.enum` takes a plain runtime `string[]`, and `zodOutputFormat` converts it to a normal JSON Schema `enum` the API already honors). Deliberately not folded into that branch — making `RecipeExtractionSchema` category-vocabulary-aware means either duplicating the schema shape or restructuring the module-level export several tests already validate generically (independent of any category list), which is its own reviewable change rather than a one-line tweak to a branch already past review.
