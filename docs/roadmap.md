...
- **Fix the staging magic-link email.** Real blocker for an actual new user signing up, not a dev-convenience gap. In progress (branch `security/otp-email-template`) — see `docs/current.md`'s Current section for the live status; the diagnosis below is superseded by that write-up, kept here only as the original framing. ~~`signInWithOtp` currently sends Supabase's default template, which only renders a clickable link, not the `{{ .Token }}` code `verifyOtp` expects the user to type.~~ Turned out deeper: neither local nor staging had ever had a code-based template written at all (not a sync gap), staging's `site_url` was still Supabase's `localhost:3000` default, `mailer_otp_length` was 8 against the UI's assumed 6, and — root blocker — Supabase rejects any email-template customization on the free tier without a custom SMTP provider, which this project has never configured (ADR-0008 flagged this as a deferred decision back in Phase 3). See `docs/current.md` carried-forward items.
