...
**Correction, observed 2026-09-01 on Expo 57.0.10 (build 4's `archive:prep` run).** Plain `npx expo prebuild -p ios` — no `--clean` — printed `Clearing ios` / `Cleared ios code` / `Creating native directory (./ios)`. It regenerated wholesale. So "updates in place" above is not something to rely on for *preserving* anything, and the paragraph's real point survives only in its second half: what protects you is that the setting lives somewhere prebuild reads. `app.json`'s `ios.appleTeamId` is why `DEVELOPMENT_TEAM = MKACSCQ588` came back on all four build configurations here rather than being lost. Anything configured **only** in Xcode has no such source and would not survive — which is what `archive:prep` means by re-checking Signing & Capabilities on both targets, and it is now the expected case rather than the `--clean` case. Mechanism not established: this was one observation, and whether prebuild always clears on this version or only when it decides the project is stale was not tested.
