...
Two things to carry rather than rediscover. **Stacked PRs get no CI**: the workflow triggers on `pull_request: branches: [main]` only, and GitHub's default event types exclude `edited`, so retargeting on merge may not fire it either — #143 has four green checks, the other three have none. And **`save_recipe` now takes an opt-in `preserveNewRecipeDraft` flag**: its create branch clears the caller's unsaved new-recipe draft, which is right for every caller except a seed performing ten creates the user did not author. Capture-and-reinsert was tried first and abandoned — a blocked `upsert_draft` does not rescan after the delete, so it hits the partial unique index and the editor's autosave swallows the failure silently (Codex, [#144](https://github.com/kraigstrong/Keepsake/pull/144)).
