...
**Starter recipes for a new library** — **shipped 2026-09-01.** All four PRs merged ([#143](https://github.com/kraigstrong/Keepsake/pull/143), [#144](https://github.com/kraigstrong/Keepsake/pull/144), [#145](https://github.com/kraigstrong/Keepsake/pull/145), [#146](https://github.com/kraigstrong/Keepsake/pull/146)), migration `20260901100000` applied to staging, `check:drift` passing. Design: [`docs/proposals/starter-recipes.md`](proposals/starter-recipes.md). Build history, the eight content defects, the reverted `save_recipe` changes and the accepted limitations: [`docs/history/cross-cutting-starter-recipes.md`](history/cross-cutting-starter-recipes.md).
...
**Rollout route decided 2026-09-02: external TestFlight**, so Beta App Review's requirements are live work rather than deferred. `docs/roadmap.md`'s Phase B now carries a **"Developer actions"** list of the six things only you can do — hosting the site, the contact mailbox, the Team ID, the reviewer account, the privacy questionnaire. The privacy policy itself is drafted and awaiting your review in [#152](https://github.com/kraigstrong/Keepsake/pull/152). Next code item is the **household-ordering trap**, the only Phase B blocker that corrupts data rather than merely annoying.
...
**Build 4 is archived and uploaded (2026-09-01), and the invite link works.** She then hit a second failure — a full-screen "Couldn't load your household" after her OTP, from a stale-token 401 on the first fetch, cleared by one Try again. Fixed in [#141](https://github.com/kraigstrong/Keepsake/pull/141). Diagnosis, and the two things that outlive it (Sentry is confirmed to deliver events; Supabase errors reach it unreadable): [`docs/history/cross-cutting-invite-blank-screen.md`](history/cross-cutting-invite-blank-screen.md).

**Build 5 is prepared but not archived (2026-09-01).** `ios.buildNumber` is 5 on `release/build-5`; its only functional content is #141. It still gates the two Phase A items that need a device rather than code: the **six-screen keyboard survey** and **Journey 3** (the two-actor shared-household walkthrough, still never done). Walk both on this build.
