...
- **Phase 15 — Cooking Mode doesn't honor the user's preferred-unit-system setting or offer an arbitrary (stepper) servings count**, unlike Recipe Detail's own scaling controls. Partially resolved 2026-08-12: the scale now defaults to the This-Week plan's servings count when this recipe is on the current confirmed plan (developer walkthrough feedback confirmed the always-1x default was a real pain point). Unit-system and arbitrary-count-stepper parity remain open — revisit if either becomes a real complaint too.
