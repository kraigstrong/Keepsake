...
- **Phase:** 14 — Apple Reminders Export. Code merged to `main` via [PR #46](https://github.com/kraigstrong/Keepsake/pull/46) (2026-08-08). Phase 13 (Grocery Generation and Review) merged earlier the same day via [PR #45](https://github.com/kraigstrong/Keepsake/pull/45). **In progress (2026-08-08): the live device-testing round for Phase 12/13/14 is underway** (developer walking This Week → Grocery Review → Reminders Export end to end on a physical device). Fixes are landing on branch `phase-12-13-14-device-testing-fixes`, not yet merged. Neither phase has had its formal `exit-phase` yet; run it once the walkthrough and its fixes are done. Neither is in the History table below for that reason.
- **Status:** Phase 13/14 build scope as previously recorded (all bullets implemented and merged; see PR #45/#46). Device-testing round found real bugs in the generation/categorization logic and UI copy, fixed on `phase-12-13-14-device-testing-fixes`: `bouillon` miscategorized as Meat, `isStaple`'s exact-match missing adjective-prefixed salt/sugar variants ("kosher salt"), `canonicalKey` not merging prep-state variants ("melted butter"), grocery-row display text leaking preparation clauses ("flour, divided"), staples mixed into their aisle category instead of their own section, and "Export" → "Send to Reminders" copy per developer product direction. 100 Jest suites / 855 tests, `tsc`/`eslint`/`prettier`/`check:client-secrets` all pass locally. Still open from this round: whether iOS's native auto-categorizing "Groceries" list type can be targeted via EventKit (researched: no public API for the list type itself; unresolved whether items added via API to an *existing* native Groceries list still get auto-sorted — needs an on-device experiment with the developer), and a few smaller product questions (optional-ingredient phrasing like "(if needed)", non-purchasable lines like "reserved pasta water", manual staple-marking — likely a later phase).
- **Next action:** Continue the live walkthrough; keep fixing/documenting on `phase-12-13-14-device-testing-fixes` as issues surface. Once the walkthrough and fixes are done, run `exit-phase` for whichever of Phase 12/13/14 are ready (they may not all close at once — Phase 14 specifically needs the physical-device evidence, ADR-0003).
