...
- **Phase:** 13 — Grocery Generation and Review. Code merged to `main` via [PR #45](https://github.com/kraigstrong/Keepsake/pull/45) (2026-08-08); formal exit (`exit-phase`, Pass/Conditional Pass decision) deliberately deferred — developer wants to get through a phase or two more before a round of live testing, rather than gating each phase on one immediately. Not yet in the History table below for that reason; add it there once `exit-phase` actually runs.
- **Status:** All eight build-scope bullets implemented and merged: ADR-0022 (design decisions), `grocery_item_selections` schema/RLS/RPC (+ `clear_grocery_item_selection`, added responding to Codex review), `canonicalKey`/`itemHash`, `categoryDictionary`/`staples`, `generateGroceryList` (scale + conservative merge + group), client data access, and the grouped review screen + This Week entry point. `prd-traceability.md` GRO-01/02/04/05/06 marked `Done (tested)`; GRO-07 `In Progress` (Phase 13's half only — no editing in review — the rest is Phase 14's export flow); GRO-03 stays `Not Started` (Phase 14). PR #45's CI (typecheck/lint/format/unit tests, migration apply/reset + pgTAP RLS tests, npm audit, gitleaks) all passed before merge; a real CI bug (ambiguous `item_hash` column inside the RPC's own `ON CONFLICT` target list) and 5 Codex findings were fixed in a follow-up commit before merge — see PR #45 for detail. Staging migrations applied (`supabase db push`, 2026-08-08) — no drift.
- **Next action:** Start Phase 14 (Apple Reminders Export) — developer's stated plan is to build ahead before the next live-testing pass. No live Simulator/device walkthrough of the Phase 13 grocery review screen has been performed yet; that and Phase 13's own `exit-phase` are both still open, to be done together in the upcoming testing round rather than one-off now.
...
- **Phase 13's own `exit-phase` and Simulator/device walkthrough** — deliberately deferred (developer decision, 2026-08-08): code is merged to `main` and staging is current, but the phase hasn't been formally exited. Do this together with Phase 12's still-open walkthrough (see above) in the upcoming live-testing round, not as a one-off.
