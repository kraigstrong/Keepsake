...
**Build 4 is archived and uploaded (2026-09-01), and the invite link works** — she opened it and reached sign-in instead of a blank screen. She then hit a second failure: a full-screen "Couldn't load your household" right after her OTP, cleared by one Try again. Fixed in [#141](https://github.com/kraigstrong/Keepsake/pull/141).

**That failure was diagnosed from real telemetry, which is the durable part.** Sentry had the sequence: the sign-in POST returned 200, a `GET /rest/v1/households` went out 44ms later and came back **401**, and a cold-start `grant_type=refresh_token` had already failed 400 before she signed in. The request beat supabase-js's new access token onto the wire and carried the stale one. Her connection was fine — so the screen's "Check your connection" copy was misleading, deliberately left alone (developer, 2026-09-01) now that the retry makes it rare. `HouseholdProvider` still logs the failed attempt when a retry recovers, so a recurrence leaves evidence.

**Sentry is confirmed to receive events** — that ambiguity is closed, by this incident rather than by a test.
