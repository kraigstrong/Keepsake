...
**Build 3 is uploaded** (`52a7161`), carrying the invite deep-link route ([#134](https://github.com/kraigstrong/Keepsake/pull/134)) and the startup-hang fix ([#135](https://github.com/kraigstrong/Keepsake/pull/135)) that build 2 predated. Both verified on a real device: the invite link opens the app instead of Unmatched Route, and airplane mode now shows "Couldn't load your household" with a working Try again rather than spinning forever. `npm run archive:prep` runs the pre-archive sequence and checks the generated project, so the `CFBundleVersion` mismatch that would otherwise surface at upload is caught in seconds.

**Build 3's invite verification was wrong, and the way it was wrong is the lesson.** The first real invitee (2026-08-31) tapped the link, the app opened, and she got a blank screen — then signed up for her own account instead. Cause: the invite route redirected to `/`, which is `(tabs)`, guarded on being fully onboarded; expo-router silently drops a *runtime* navigation to a screen whose `Stack.Protected` guard is false (it only falls back for the initial URL). So both states a real invitee is in — signed out, and signed in mid-onboarding — landed on a mounted invite route rendering null. It read as verified because it was only ever tapped from an already-onboarded phone, the one state where `/` exists. **Verify an invite flow from a device that has never joined a household, or you have verified nothing.** Fixed on `fix/invite-blank-screen`, three `renderRouter` suites pinning all three branches. **Needs a build 4 before the two-person test can resume.**
