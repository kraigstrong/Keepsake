...
**The one open item that is not code:** the *hosted* Supabase project's auth settings are unverified. `config.toml` governs only the local stack; if the hosted project mirrors it (`enable_signup = true`, `enable_confirmations = false`), anyone with the publishable key — extractable from the bundle — can create an unverified account. They land in an isolated empty household, so it is not a data-exposure path, just unbounded account creation. Check the dashboard before invites go out.
...
**Two migrations have never reached staging** (found 2026-08-29 via `supabase migration list`): `20260827120000` (weekly-plan locking) and `20260828100000` (cooking-note cap). The first is gate 2's own fix, recorded above as closed — closed in code, never applied to the hosted database. Applying both is Phase A's first blocker. And the build currently in TestFlight (`v1.0.0` at `5d1afff`) predates every fix in [#128](https://github.com/kraigstrong/Keepsake/pull/128), so build 2 is required before anyone else installs it.
