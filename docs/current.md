...
- **Phase:** 15 — Cooking Mode and Cooking History. Phases 12, 13, and 14 have all formally exited (12: Conditional Pass, 2026-08-07; 13 and 14: Pass, 2026-08-10 — see History table). Build scope complete on branch `phase-15-cooking-mode` (11 commits, not yet pushed), not yet exited.
- **Status:** Full build scope landed: ADR-0024 (design decisions), `cooking_events` server schema/RLS/RPCs, local schema v10 (`cooking_sessions`/`cooking_event_outbox`), checklist-state and offline-outbox domain logic, server operations wired into `app/_layout.tsx`'s existing foreground/reconnect drain pattern, `useCookingSession` hook, the Cooking Mode screen itself (single scrolling screen, scaling presets, keep-awake, VoiceOver announcements, reachable via a new "Start Cooking" button on Recipe Detail), the Done Cooking confirmation sheet (optional note, connectivity-gated remove-from-This-Week toggle), and a Cooking History section on Recipe Detail (hidden when empty, newest-first). An adversarial security/maintainability/product review (developer-requested, before building further) found and fixed two real gaps: a failed cooking-event submission had no retry path at all (fixed — now retries automatically on every foreground/reconnect, ADR-0024 amendment) and no threat-model entry existed for this phase's new RPC/queue surface (fixed — T22, with one disclosed-not-closed gap: no length cap on `cooking_events.note`). Test debugging surfaced the same React 19 "act must be awaited" class of issue three more times (opening a Modal via state update, typing then immediately pressing confirm, toggling then immediately confirming) — each one, left unfixed, silently corrupted every later test in the same file; all fixed. 107 suites / 917 tests passing, `tsc`/`eslint`/`prettier`/`check:client-secrets` clean. pgTAP suite (`cooking_event_rpcs.test.sql`) not run locally (no Docker), same as every phase since 12 — CI is the real gate. `docs/prd-traceability.md` updated: COOK-01..06, NOTE-01..04, REC-05, OFF-03/05 all `Done (tested)`.
- **Next action:** Push `phase-15-cooking-mode` and open a PR (see `pr-ready`). Once merged, ADR-0003 requires a **physical-device** demonstration before this phase can formally exit (screen-awake/real-kitchen-use class, same as Phase 14) — not yet performed. Known, disclosed, not-yet-fixed items to weigh at exit: no length cap on `cooking_events.note`; Cooking Mode doesn't honor the user's preferred-unit-system setting or offer an arbitrary servings count the way Recipe Detail does (silently narrower than Recipe Detail's own scaling controls); Reset checklist has no confirm/undo; "Start Cooking" was made the primary action on Recipe Detail (Add to This Week demoted to secondary) as an unconfirmed UI-emphasis call.
- **Blocked on:** Nothing.
