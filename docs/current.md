...
- **Phase:** 14 — Apple Reminders Export. Build scope complete on branch `phase-14-reminders-export`, not yet pushed/PR'd. Phase 13 (Grocery Generation and Review) merged to `main` via [PR #45](https://github.com/kraigstrong/Keepsake/pull/45) (2026-08-08). Neither phase has had its formal `exit-phase` yet — deliberately deferred, developer wants to get through a phase or two more before a round of live testing rather than gating each phase on one immediately. Neither is in the History table below for that reason; add both once `exit-phase` actually runs.
- **Status:** All eight Phase 14 build-scope bullets implemented across 6 commits: ADR-0023 (design decisions — local-only export bookkeeping, reusing ADR-0022's `itemHash` as the dedup/retry key), local `grocery_exports` table (schema v9, survives sign-out like `import_outbox`), `exportRecords.ts` (dedup read/write), `reminders.ts` evolved (`requestReminderPermission()` now returns the full `PermissionResponse`; added `openReminders()`), `exportGroceries.ts` (sequential per-item export with partial-failure handling), and `GroceryExportPanel` (point-of-use permission, canAskAgain-driven recovery, progress, result summary, Open Reminders, Retry failed) wired into `GroceryReviewScreen`. `prd-traceability.md` GRO-03/GRO-07 now `Done (tested)`; `docs/threat-model.md` gained T21. 100 Jest suites / 826 tests, `tsc`/`eslint`/`prettier`/`check:client-secrets` all pass locally — no pgTAP this phase (no server component at all, ADR-0023's central decision).
- **Next action:** Push `phase-14-reminders-export` and open a PR (`pr-ready`) once the developer wants it up for review — not done yet this session. ADR-0003 requires a **physical device** (not just Simulator) for this phase's own exit gate — ADR-0003 already scoped Phase 14 as physical-device-required — so it can't formally exit without one; bundle that check into the same upcoming live-testing round as Phase 12's and Phase 13's still-open walkthroughs rather than a one-off.
...
- **Phase 13 and Phase 14's own `exit-phase` and device walkthroughs** — deliberately deferred (developer decision, 2026-08-08): Phase 13 is merged to `main` with staging current; Phase 14 is built but not yet even pushed. Neither has been formally exited. Do these together with Phase 12's still-open walkthrough (see above) in one upcoming live-testing round rather than one-off per phase — Phase 14 specifically needs a **physical device**, not just Simulator (ADR-0003).
