...
Membership is fixed once joined — leaving a household is out of scope for MVP (see ADR-0004).
...
- React Native
- TypeScript
- iOS only for MVP. A web build (Expo web) is used for internal development and testing of flows that don't depend on native device features — it is not a shipping target and does not need feature parity (see ADR-0002).
...
- Keep Anthropic, Supabase service-role, database, deployment, signing, and other privileged credentials out of client applications.
...
## 32. Repository & Process

Engineering process, phase sequencing, and delivery discipline live in [`docs/execution-plan.md`](./execution-plan.md), not in this document. Agent operating rules (commit discipline, when to ask for a decision, security checkpoints) live in the repository root [`CLAUDE.md`](../CLAUDE.md).
